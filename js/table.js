...
function baseCanvas(img){const max=2200,scale=Math.min(1,max/Math.max(img.naturalWidth,img.naturalHeight)),cv=document.createElement('canvas');cv.width=Math.max(1,Math.round(img.naturalWidth*scale));cv.height=Math.max(1,Math.round(img.naturalHeight*scale));const x=cv.getContext('2d',{willReadFrequently:true});x.fillStyle='#fff';x.fillRect(0,0,cv.width,cv.height);x.drawImage(img,0,0,cv.width,cv.height);return cv;}
function cropCanvas(source,r){const cv=document.createElement('canvas'),sx=Math.round(source.width*r.x),sy=Math.round(source.height*r.y);cv.width=Math.max(1,Math.round(source.width*r.width));cv.height=Math.max(1,Math.round(source.height*r.height));const x=cv.getContext('2d',{willReadFrequently:true});x.fillStyle='#fff';x.fillRect(0,0,cv.width,cv.height);x.drawImage(source,sx,sy,cv.width,cv.height,0,0,cv.width,cv.height);return cv;}
...
async function detectSource(source,region){const hits=[];for(const item of [{pass:'original',cv:source},{pass:'gray',cv:enhancedCanvas(source)}]){try{for(const r of await labelDetector.detect(item.cv)){const value=barcodeText(r.rawValue);if(!value)continue;const box=r.boundingBox||{x:0,y:0,width:item.cv.width,height:item.cv.height};hits.push({value,region,pass:item.pass,x:box.x/item.cv.width,y:box.y/item.cv.height,width:box.width/item.cv.width});}}catch(e){console.warn('Detection pass skipped',region,item.pass,e);}}return hits;}
function combineHits(hits){const map=new Map();for(const h of hits){const key=`${h.region}|${h.value}`;if(!map.has(key))map.set(key,{...h,count:0});map.get(key).count++;}return[...map.values()].sort((a,b)=>b.count-a.count);}
async function targetedDetect(img){const base=baseCanvas(img),regions=[{name:'part',x:.05,y:.31,width:.73,height:.40},{name:'quantity',x:0,y:.60,width:.42,height:.38},{name:'quantity',x:0,y:.52,width:.50,height:.46}],hits=[];for(const r of regions)hits.push(...await detectSource(cropCanvas(base,r),r.name));try{for(const result of await labelDetector.detect(base)){const value=barcodeText(result.rawValue);if(value)hits.push({value,region:'full',pass:'original',x:(result.boundingBox?.x||0)/base.width,y:(result.boundingBox?.y||0)/base.height,width:(result.boundingBox?.width||0)/base.width});}}catch(e){console.warn('Full pass skipped',e);}return combineHits(hits);}
function classifyHits(items){let part='',qty='';for(const i of items){const p=taggedValue(i.value,'P'),q=taggedValue(i.value,'Q').replace(/EA$/,'');if(p&&validPart(p))part=normalizeDetectedPart(p);if(q&&validQty(q))qty=String(Number(q));}if(!part){const candidates=items.filter(i=>i.region==='part').map(i=>({...i,value:normalizeDetectedPart(i.value)})).filter(i=>validPart(i.value)&&!/^[(]?[QSVTM][):=\-]?/.test(i.value));candidates.sort((a,b)=>Number(knownTablePart(b.value))-Number(knownTablePart(a.value))||b.count-a.count||b.width-a.width||b.value.length-a.value.length);part=candidates[0]?.value||'';}if(!part){const candidates=items.filter(i=>i.region==='full').map(i=>({...i,value:normalizeDetectedPart(i.value)})).filter(i=>validPart(i.value)&&!/^[(]?[QSVTM][):=\-]?/.test(i.value));candidates.sort((a,b)=>Number(knownTablePart(b.value))-Number(knownTablePart(a.value))||b.width-a.width);part=candidates[0]?.value||'';}if(!qty){let candidates=items.filter(i=>i.region==='quantity'&&validQty(i.value)&&barcodeText(i.value).replace(/EA$/,'').length<=7);candidates.sort((a,b)=>b.count-a.count||barcodeText(a.value).length-barcodeText(b.value).length||a.x-b.x);if(!candidates.length)candidates=items.filter(i=>i.region==='full'&&i.x<.52&&i.y>.48&&validQty(i.value)&&barcodeText(i.value).replace(/EA$/,'').length<=7);if(candidates.length)qty=String(Number(barcodeText(candidates[0].value).replace(/EA$/,'')));}return{part:normalizeDetectedPart(part),qty};}
...
function updateScannerUseButton(){const part=normalizeDetectedPart(document.getElementById('scannerPartValue').value),qty=document.getElementById('scannerQtyValue').value.trim();document.getElementById('useScannerBtn').disabled=!(validPart(part)&&validQty(qty));document.getElementById('scannerPartWarning').hidden=!part||knownTablePart(part);}
function resetLabelPhoto(){labelBusy=false;const input=document.getElementById('labelPhotoInput');if(input)input.value='';if(labelUrl)URL.revokeObjectURL(labelUrl);labelUrl='';document.getElementById('photoPanel').hidden=true;document.getElementById('scannerPartValue').value='';document.getElementById('scannerQtyValue').value='';document.getElementById('scannerRawValue').textContent='Ninguno';document.getElementById('scannerPartWarning').hidden=true;document.getElementById('useScannerBtn').disabled=true;labelMessage('Toma una foto clara de la etiqueta completa.');}
